...
this.messageListener = event => {
  ...
  try {
    const parsedData = JSON.parse(messageData.replace('chatwoot-widget:',''));
    if (parsedData?.eventIdentifier === 'chatwoot:on-message') {
      window.ReactNativeWebView.postMessage('chatwoot-widget:' + JSON.stringify(parsedData));
    }
  } catch (e) {
